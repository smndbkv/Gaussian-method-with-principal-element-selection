...
inline bool main_element(double *a, int n, int m, int s, double *c, double *c_inv,
                         int &res_i, int &res_j, double nrm_a, double &min_norm, int p, int q)
{
...
gauss_status gauss_method(args *Arg, double *c, double *g, double *d, double *f)
{
  ...
  {
    // printf("----------Step %d-----------\n", s);
    if (!main_element(a, n, m, s, c, g, Arg->min_i[q], Arg->min_j[q], nrm_a, Arg->min_norm[q], p, q))
    {
    ...
    {
      // printf("%d\n", (int)err[i]);
      if (err[i] == gauss_status::DONE && Arg->min_norm[ind] > Arg->min_norm[i])
      {
        // printf("i  = %d\n", i);
        ind = i;
      }
